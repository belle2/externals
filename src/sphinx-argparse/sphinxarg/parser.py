...
def _try_add_parser_attribute(data, parser, attribname, targetname=None):
    ...
    if targetname is None:
        targetname = attribname
    if len(attribval) > 0:
        data[targetname] = attribval % {"prog": data.get("prog", "")}
...
def parse_parser(parser, data=None, **kwargs):
    ...
    # if add_subparsers had a title or description attribute then the subparsers
    # get their own ArgumentGroup. Let's extract title and description from
    # there
    if "children" in data and parser._subparsers != parser._positionals:
        _try_add_parser_attribute(data, parser._subparsers, "title", "children_title")
        _try_add_parser_attribute(data, parser._subparsers, "description", "children_description")
